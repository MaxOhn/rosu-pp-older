...
use rosu_map::section::{
    general::GameMode,
    hit_objects::{
        BorrowedCurve, CurveBuffers, PathControlPoint, SliderEvent, SliderEventType,
        SliderEventsIter,
    },
};
use rosu_pp::{
...
impl<'a> JuiceStream<'a> {
    ...
    pub fn new(
        ...
        let beat_len = util::control_points::timing_point_at(&map.timing_points, start_time)
            .map_or(TimingPoint::DEFAULT_BEAT_LEN, |point| point.beat_len);

        let slider_velocity =
            util::control_points::difficulty_point_at(&map.difficulty_points, start_time)

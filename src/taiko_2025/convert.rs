...
use crate::util::{
    self, float_ext::FloatExt, get_precision_adjusted_beat_len, random, sort::TandemSorter,
};
    ...
    while idx < map.hit_objects.len() {
        match map.hit_objects[idx].kind {
            ...
            HitObjectKind::Slider(ref slider) => {
                ...
                let slider_velocity = util::control_points::difficulty_point_at(
                ...
                if !FloatExt::almost_eq(last_scroll_speed, slider_velocity, f64::EPSILON) {
                    let curr_kiai =
                        util::control_points::effect_point_at(&map.effect_points, obj.start_time)
...
fn should_convert_slider_to_taiko_hits(map: &Beatmap, params: &mut SliderParams<'_>) -> bool {
    ...
    let timing_beat_len = util::control_points::timing_point_at(&map.timing_points, *start_time)

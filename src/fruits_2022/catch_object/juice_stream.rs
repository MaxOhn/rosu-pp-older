...
use rosu_map::section::hit_objects::{
    BorrowedCurve, CurveBuffers, PathControlPoint, SliderEvent, SliderEventType, SliderEventsIter,
};
use rosu_pp::{
    model::{
...
use crate::{
    fruits_2022::{attributes::ObjectCountBuilder, PLAYFIELD_WIDTH},
    util::control_points::{difficulty_point_at, timing_point_at},
};
...
impl<'a> JuiceStream<'a> {
    ...
    pub fn new(
        ...
        let beat_len = timing_point_at(&map.timing_points, start_time)
            .map_or(TimingPoint::DEFAULT_BEAT_LEN, |point| point.beat_len);

        let slider_velocity = difficulty_point_at(&map.difficulty_points, start_time)
            .map_or(DifficultyPoint::DEFAULT_SLIDER_VELOCITY, |point| {
                point.slider_velocity
            });

        let path = BorrowedCurve::new(
            GameMode::Catch,
